            ...
            if path.endswith(".json"):
                with open(path, "r", encoding="utf-8") as f:
                    ...
                    for x in reldata["venues_id"]:
                        for y in reldata["venues_id"][x]:
                            findex+=1
                    
                    df_venues_id=pd.DataFrame(columns=["id","venue_id"],index=range(findex))
                    ind=0
                    for x in reldata["venues_id"]:
                        for y in reldata["venues_id"][x]:
                            df_venues_id.iloc[ind]=(x,y)
                            ind+=1
                    df_venues_id = df_venues_id.rename(columns={"id":"doi","venue_id":"venueID"})     
...
"""]    
    ...
    SELECT ?Cited ?internalID ?doi ?title ?type ?publicationYear ?chapterNumber ?issue ?volume ?venueID ?all_authors_id ?id_all_references
    WHERE{
        VALUES ?type {
        schema:ScholarlyArticle schema:Chapter schema:Article}
        ?internalID rdf:type ?type. 
...
#TriplestoreQueryProcessor.getMostCitedVenue(TriplestoreQueryProcessor)
    ...
    def getVenuesByPublisherId (self, publisher_id):
        ...
        query_5= ["""
                PREFIX rdf:  <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
                PREFIX schema: <https://schema.org/>
                SELECT ?internalID ?venueID ?type ?title ?event ?publisherID
                WHERE{
                 VALUES ?type {
...
class GenericQueryProcessor(object):
    ...
    def getVenuesByPublisherId(self, publisherId):
        ...
        for row_idx,row in result.iterrows():
            x=Venue(identifiers=row["venueID"],title=row["title"],publisher=row["publisherID"])
            self.finalresultlist.append(x)
